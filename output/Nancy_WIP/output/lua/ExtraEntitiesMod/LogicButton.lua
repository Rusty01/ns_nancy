...
LogicButton.kSound = PrecacheAsset("sound/ns1.fev/button/button")
...
function LogicButton:OnUse(player, elapsedTime, useAttachPoint, usePoint, useSuccessTable)
    ...
    if Server then   
        local timeOk = ((Shared.GetTime() + 0) >= self.timeLastTriggered)
        ...
        if self.enabled and timeOk then
            ...
            if teamOk then
                ...
                if typeOk then
                    ...
                    self:AddTimedCallback(function()  self:TriggerOutputs(player) end, 1)
                    self.timeLastTriggered = Shared.GetTime()
                    StartSoundEffectForPlayer(LogicButton.kSound, player)
                end
