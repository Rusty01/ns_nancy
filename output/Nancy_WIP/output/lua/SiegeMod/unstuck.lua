...
function SiegeUnstickPlayer(self, Player, Pos)
	...
	local Range = self.Config.DistanceToCheck
        ...
        local NearbyDoors = GetEntitiesWithinRange("SiegeDoor", Pos, Range)
        local NearbyMoveables = GetEntitiesWithinRange("FrontDoor", Pos, Range)
        local NearbyBreakables = GetEntitiesWithinRange("LogicBreakable", Pos, Range)
        local Naughty = false
