...
if Server then
    ...
    function LogicTrigger:CheckTrigger(enterEnt)
        ...
        local timeOk = self.timeLastTriggered == nil or ( Shared.GetTime() > self.timeLastTriggered + .5) or self.ignorecheck and not enterEnt.isaNpc
    ...
    function LogicTrigger:OnTriggerEntered(enterEnt, triggerEnt) 
        if enterEnt:isa("Player") then //and (self.triggerStyle == 0 or self.triggerStyle == nil) then
    ...
    function LogicTrigger:OnTriggerExited(exitEnt, triggerEnt)
        if exitEnt:isa("Player") then //and self.triggerStyle == 1 then
